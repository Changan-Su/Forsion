/**
 * 版本号比较(纯函数,零依赖)。桌面 updater 与移动端更新检查共用这一份。
 *
 * ⚠️ 住 `desktop/shared/` 而不是 `desktop/electron/`:web 镜像的构建上下文只 COPY 了
 * desktop/frontend、desktop/shared、lcl、mobile/src、web(见 web/Dockerfile),而 web 整份复用
 * mobile 壳 —— 移动端若从 `desktop/electron/**` 拿这个函数,本机与 check-unit 全绿、build-web 才红。
 */
