...
| 3.5 thin | `--gl-thin`(50%) | 输入卡 + 菜单/各类浮层 | 身后有内容在动,糊得出来才算玻璃 |
| 4 float | `--gl-float`(84%) | 弹窗、命令面板、toast | 密集文字的大面板,再透就影响读 |

浓度旋钮(chrome/pane/float/模糊半径/饱和度)已接进**设置 → 主题 → 选中卡下方的滑块**
(`theme.json` 的 `settings[]`,宿主把值写成 `:root` 内联变量)。
`--gl-thin` **不接旋钮**,写字面值 —— 带 `--gl-*-pct` 的写法专留给 `theme.json` 声明过的档位,
...
- **侧栏糊不出来(不是 bug,是物理)**:CSS 的 `backdrop-filter` 只能糊页面自己画出来的像素。
  ...
  现象是「输入卡后面的正文原样清晰地透过来」。故侧栏组内的输入卡/菜单**回到 float 那档靠浓度**
  (`.dv-groupview:has(.wb-tab--icon)` 选择器)。主区能糊是因为身后有实色纸卡。
...
仪器:`npm run check:chatside` 的 24/25 两条(输入卡主区薄档 / 侧栏加厚、二级菜单是不是薄档磨砂)。
