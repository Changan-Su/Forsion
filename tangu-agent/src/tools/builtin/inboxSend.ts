  ...
  tools: () => [
    {
      ...
      definition: {
        ...
        function: {
          name: 'inbox_send',
          // E1 三段式(§五)。ponytail: 上限 905B,不是 600 —— 下面的 ```forsion-task 围栏是**被解析的
          // 格式契约**(桌面端 TaskCards 按它解析),连同示例字段逐字留着,只压它周围的散文。
          // 再往下压就得动围栏本身 = §六 P08 说的「未标注的产品收窄」,不做。
          description:
            "Send the user a message in their inbox (the app's notification center) — results, reminders or " +
            'follow-ups they should notice outside this conversation, not a substitute for replying here. ' +
            `The body (max ${MAX_BODY} chars) renders as an Amadeus note: a short document leading with the conclusion, full markdown ` +
            '(tables, callouts, math, code), `![[note]]` vault embeds, absolute paths for other files. ' +
            'For a one-click follow-up, end the body with up to two task cards (run in a new session, hand to Muse, or ignore):\n' +
            '```forsion-task\ntitle: Short task title\ntldr: One line for the user (optional)\n---\n' +
            'Self-contained brief for a fresh session: paths, symptoms, acceptance criteria.\n```\n' +
            'Add `track: true` when it needs follow-up over days rather than now. ' +
            'A ```forsion-button block can reference a manual automation rule id.\n' +
            'Delivery is immediate — for a future or recurring reminder use manage_automation instead.',
          parameters: {
            type: 'object',
            properties: {
