 ...
 *
 * 三种拖源(会话 / 笔记 / 文件)统一成一个 DataTransfer 载荷,聊天区落下时转成**草稿里的文本引用**
 * ——与既有的 `[[` 笔记选择器、OS 文件拖入完全同一套,agent 拿到路径/id 后自己用工具去读。
 *
...
/**
 * 一条引用落进草稿的文本(纯函数,仪器钉这里)。末尾恒带一个空格,便于连拖多条。
...
/** 一次拖拽(可能多条)拼成的插入文本。 */
export function refsToText(refs: ChatRef[], vaultRoot: string): string {
  return refs.map((r) => refToText(r, vaultRoot)).join('')
}
