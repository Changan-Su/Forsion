...
async function main() {
  ...
  try {
    ...
    // 设置画在独立浮窗里(Floating Panel 化 2026-09-20):开窗前先 arm window 事件,面板 locator 一律走浮窗 page。
    const settingsOpening = running.waitForEvent('window', { predicate: page => page !== win, timeout: 30_000 })
    await win.keyboard.press('Meta+,')
    const settings = await settingsOpening
    await settings.waitForLoadState('domcontentloaded')
    await settings.locator('.settings-nav-list button').filter({ hasText: '设备权限' }).click()
    await settings.waitForSelector('.desktop-permissions')
    check('settings keeps a permanent permissions entry', await settings.locator('[data-permission]').count() === 5)
    await settings.screenshot({ path: '/tmp/forsion-permissions-settings-zh.png', animations: 'disabled' })
    // 换英文前必须把浮窗关掉:面板按 id 复用,开着的那个既不会重载成英文,再敲热键也不会有新 window 事件。
    ...
    await win.reload()
    await win.waitForSelector('.dv-groupview', { timeout: 30_000 })
    const settingsOpeningEn = running.waitForEvent('window', { predicate: page => page !== win, timeout: 30_000 })
    await win.keyboard.press('Meta+,')
    const settingsEn = await settingsOpeningEn
    await settingsEn.waitForLoadState('domcontentloaded')
    await settingsEn.locator('.settings-nav-list button').filter({ hasText: 'Device permissions' }).click()
    await settingsEn.waitForSelector('.desktop-permissions')
    await settingsEn.screenshot({ path: '/tmp/forsion-permissions-settings-en-dark.png', animations: 'disabled' })
    check('English permissions text contains no untranslated Chinese', !/[\u3400-\u9fff]/.test(await settingsEn.locator('.desktop-permissions').innerText()))
    const overflow = await settingsEn.locator('.desktop-permissions').evaluate(el => el.scrollWidth > el.clientWidth + 1)
