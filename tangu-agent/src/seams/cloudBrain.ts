...
export interface BuildPayloadOpts {
   ...
   *  来自 run 的 input.client(已过 /agent/runs 的白名单闸);runtime 接缝会让压缩/Historian/vision/
   *  子代理等派生调用自动继承。仅无用户 run 来源的服务端定时任务保持为空。 */
  client?: string;
  temperature?: number;
  maxTokens?: number;
  tools?: Tool[];
...
export interface InboxBrain {
   ...
   *  失败抛错;错误对象带 status(410=已过期/403=非目标或不满足领取条件/404=不存在)。 */
  claimBroadcast(broadcastId: string, client?: string): Promise<{ claimed: boolean; alreadyClaimed?: boolean }>;
}
