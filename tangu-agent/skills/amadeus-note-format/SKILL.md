...
任一份是 `.excalidraw.md` / `.db` / `.mindmap.md`,或 frontmatter 里有 `amadeus_canvas` → **不合并,两份都不要动**。画布几何和表格没有可手写的合并法,合了就是毁档。告诉用户在 app 里二选一(保留一份、删另一份),然后停。
