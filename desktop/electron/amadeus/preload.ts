...
const api: AmadeusApi = {
  ...
  listMarks: () => ipcRenderer.invoke(IPC.listMarks),
  patchMark: (pagePath: string, raw: string, occ: number, next: string) => ipcRenderer.invoke(IPC.patchMark, pagePath, raw, occ, next),
  ...
  writeDatabaseCas: (dbPath, data, baseVersion) => ipcRenderer.invoke(IPC.dbWriteCas, dbPath, data, baseVersion),
