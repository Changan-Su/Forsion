...
export function webviewUrlAllowed(raw: string): boolean {
  ...
  // ⚠️ IPv4-mapped IPv6 会绕过下面**所有**私网判定:`http://[::ffff:127.0.0.1]/` 被 URL 规范化成
  // `[::ffff:7f00:1]`(十六进制形态),v4 正则匹配不上、fc/fd/fe80 也匹配不上 → 一路放行到环回,
  // `[::ffff:169.254.169.254]` 更是直通云元数据。先折回点分十进制,再走原有判定(Codex 评审实测)。
  // 十进制/八进制整数形态(`http://2130706433/`、`http://0177.0.0.1/`)不用管:URL 自己会规范化成
  // `127.0.0.1`,已被 v4 分支拦住。
  const mapped = /^::ffff:(.+)$/.exec(h)
  if (mapped) {
    const r = mapped[1]
    if (/^\d{1,3}(\.\d{1,3}){3}$/.test(r)) h = r
    else {
      const g = /^([0-9a-f]{1,4}):([0-9a-f]{1,4})$/.exec(r)
      if (g) {
        const hi = parseInt(g[1], 16)
        const lo = parseInt(g[2], 16)
