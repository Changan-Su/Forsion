...
约每 16ms 以固定速度计算直线位移，不使用原生 `animate=true`、缓动或弹簧。面板避开光标并夹紧到其所在显示器的工作区，跟随时停用贴边折叠和鼠标命中。输入结束后完成最后一段位移；手动 Mini 恢复交互，临时观察窗口保留到本轮运行结束或 Forsion 重新获焦。隐藏或关闭后不再移动。
