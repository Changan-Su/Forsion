 ...
 * 注册表 —— config.toml 解析/序列化往返(数组表尾置、未知 mode 回落、去重、lead 兜底)、buildTeamDef 校验(≥2 成员)、列表顺序按 .meta.json;
 * 路由 —— CRUD、POST slug 唯一化、session/open 幂等与形状(teamSlug + groupAgents + teamMode + cwd=团队 Library)、云端 404;
...
describe('teamRegistry(纯函数)', () => {
  it('解析:未知 mode 回落 meeting;成员去重 + 非法 slug 剔除;lead 不在成员表 → 第一位;max_rounds 钳 1..30;TEAM.md 截 16KB', () => {
    const toml = `name = "Team abc"\nmode = "chaos"\nmax_rounds = 99\nlead = "nobody"\n[[members]]\nslug = "ario"\nrole = "Architect"\n[[members]]\nslug = "ario"\n[[members]]\nslug = "Bad Slug"\n[[members]]\nslug = "bo"\n`;
    const def = parseTeamConfig('abc', toml, 'x'.repeat(20000));
    expect(def).toMatchObject({ slug: 'abc', name: 'Team abc', mode: 'meeting', maxRounds: 30, lead: 'ario', members: [{ slug: 'ario', role: 'Architect' }, { slug: 'bo', role: '' }] });
    expect(def.doc.length).toBe(16 * 1024);
    expect(def.libraryDir).toBe(teamLibDirOf('abc'));
  });
  it('序列化:标量在前、[[members]] 尾置,往返不丢', () => {
    const def = buildTeamDef('abc', null, { name: 'Team abc', mode: 'collab', maxRounds: 5, lead: 'bo', avatar: '🛰️', members: [{ slug: 'ario', role: 'API' }, { slug: 'bo' }], doc: '# T' });
    const toml = serializeTeamConfig(def);
    expect(toml.indexOf('[[members]]')).toBeGreaterThan(toml.indexOf('max_rounds'));
    expect(toml.lastIndexOf('created_at')).toBeLessThan(toml.indexOf('[[members]]'));
    const back = parseTeamConfig('abc', toml, '# T');
    expect(back).toMatchObject({ name: 'Team abc', mode: 'collab', maxRounds: 5, lead: 'bo', avatar: '🛰️', members: [{ slug: 'ario', role: 'API' }, { slug: 'bo', role: '' }], doc: '# T' });
  });
  it('buildTeamDef:少于 2 名有效成员 / 无名 / 非法 slug 抛错;members 未传保留既有', () => {
    ...
    const ex = buildTeamDef('t', null, { name: 'T', members: [{ slug: 'a' }, { slug: 'b' }], mode: 'collab' });
    expect(buildTeamDef('t', ex, { name: 'T2' })).toMatchObject({ name: 'T2', mode: 'collab', members: ex.members });
  });
...
beforeAll(async () => {
  ...
  const fakeLlm: any = {
    ...
    streamProviderCompletion: async (o: any) => {
      ...
      if (o.payload.toolChoice?.function?.name === 'cast_vote') return { content: '', reasoning: '', toolCalls: [{ id: 'v', type: 'function', function: { name: 'cast_vote', arguments: JSON.stringify({ end: true, reason: 'r' }) } }], usage };
      return { content: 'ok', reasoning: '', toolCalls: [], usage, finishReason: 'stop' };
    },
  };
...
describe('routes /agent/teams', () => {
  it('POST 建团队(落盘 config.toml + TEAM.md + Library/)、同名再建 slug 递增、GET 列表按 .meta.json 顺序、PATCH 逐字段、DELETE', async () => {
    const a = await call('POST', '/agent/teams', { name: 'Team abc', members: [{ slug: 'ario', role: 'API' }, { slug: 'bo' }], mode: 'collab', doc: '# Team abc\nAlpha owns API.' });
    expect(a.status).toBe(200);
    expect(a.body.team).toMatchObject({ slug: 'team-abc', mode: 'collab', members: [{ slug: 'ario', role: 'API' }, { slug: 'bo', role: '' }] });
    ...
    const p = await call('PATCH', '/agent/teams/team-abc', { mode: 'meeting', maxRounds: 3 });
    expect(p.body.team).toMatchObject({ mode: 'meeting', maxRounds: 3, name: 'Team abc', members: a.body.team.members });
  ...
  it('session/open:幂等;形状 = projectless + teamSlug + groupChat + 成员表 + teamMode + cwd=团队 Library + preset null;过 validSessionFacts', async () => {
    ...
    expect(s.agent_config).toEqual({ teamSlug: 'team-abc', groupChat: true, groupAgents: ['ario', 'bo'], teamMode: 'meeting', groupMaxRounds: 3, execMode: 'host', cwd: teamLibDirOf('team-abc'), preset: null });
  ...
  it('loop:存值 teamSlug 的会话,run 不带 groupAgents/teamMode 也按成员表进群聊;TEAM.md + role 进每位成员 system', async () => {
    ...
    expect(speakers).toEqual(['ario', 'bo']); // 3 轮上限但首轮投票全员结束 → 1 轮
    const ended = ev.find((e) => e.type === 'group_ended');
    expect(JSON.parse(ended!.payload).mode).toBe('meeting');
