...
async function main() {
  ...
  try {
    ...
    await win.waitForSelector('.settings-main', { timeout: 8000 })
    const nav = win.locator('.settings-nav')
    const general = nav.getByRole('button', { name: '常规设置', exact: true }).first()
    if (await general.count().catch(() => 0)) { await general.click().catch(() => {}); await win.waitForTimeout(600) }
    const panel = win.locator('.settings-panel', { hasText: '有会话运行时阻止休眠' }).first()
    ...
    await win.screenshot({ path: path.join(SHOT_DIR, 'keepawake-general-page.png') })
    await sw.click()
    await win.waitForFunction(() => document.querySelector('.settings-panel [aria-label="有会话运行时阻止休眠"]')?.getAttribute('aria-checked') === 'true', null, { timeout: 5000 }).catch(() => {})
    ...
    await win.locator('.settings-nav button:text-is("返回应用"), .settings-nav button:text-is("Back to app")').first().click({ timeout: 5000 }).catch(() => win.keyboard.press('Escape'))
    await win.waitForTimeout(800)

    // T3 run 结束 → 放
    check('T2e 设置已关回应用', (await win.locator('.settings-main').count()) === 0)
