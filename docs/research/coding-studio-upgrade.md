---
title: Coding Studio 升级调研
description: Google AI Studio 等工具的能力对照、Forsion 工作流设计、原生面板与动效适配及验证边界。
---
...
## 2026-09-07 续改：原生 Temp View、布局与真实动效

针对页面拥挤与缺少动效的反馈，起步页缩小主表单、折叠可选要求与目录，把导入保留在标题区；模板按容器宽度呈现三、二、一列。工作台将项目与模式放在顶部、预览操作放在地址栏、项目工具与运行状态放在底部，新布局默认收起右栏。

简报、版本、验收、问题与预览设置通过 `ViewProps.extendView` 打开原生临时视图。默认按任务选择右侧或底部，各工具可移至左、右、底部，记住本项目中的手动位置。稳定 React portal 保留工具 DOM 与草稿，同一入口聚焦已有实例；生命周期关闭仅处理自己的 handle，临时视图不进入持久布局。嵌入卡片没有原生控制器时使用局部工具视图。

工具沿用 Forsion Command 通道：命令面板提供五个入口，Agent 可用 `coding-studio.open-tool` 按明确项目、工具与面板位置打开界面；校验未知参数、过期项目与卸载后的调用，不把打开视图变成保存、恢复或发送请求。

动效覆盖起步分区进入、可选表单双向展开、模式指示与真实代码列宽、设备宽度、局部修改表单、工具内容与原生面板开合。Electron 仪器逐帧采样真实宽高，要求出现多帧中间值；系统「减少动态效果」同时禁用 CSS 与原生面板尺寸补间。可选 `STUDIO_MOTION_VIDEO=1 npm run e2e:coding` 会保存原生窗口的操作帧，供生成录屏。

Temp View 的增删会让 Dockview 重整 DOM 祖先。实测即使 React 节点身份保留，Electron 仍会销毁被摘挂的网页 guest；`moveBefore` 搬移网页或祖先同样不能保留 guest 内存。因此预览独立挂在稳定 shell 容器，原面板只保留定位锚点，连续同步几何、裁剪、缩放、可见性和焦点。关闭项目才销毁稳定宿主。拖拽面板时让命中穿透到原锚点，使 Forsion 原有分屏落点计算继续生效。

[Electron 官方说明](https://www.electronjs.org/docs/latest/api/webview-tag/)确认网页由独立进程中的 iframe 承载，宿主不能直接订阅其中的鼠标与键盘事件。本次将真实网页内存保留、原生面板焦点、拖放提示、界面缩放和浮层命中作为独立回归项，避免只检查 React DOM 或截图就判断通过。

续改的真实 Electron 验收 **73/73 通过**：新增原生面板三侧迁移、相同入口重开、关闭后草稿保留、guest 内存保留、原生网页点击激活面板、拖放提示、项目退出清理、命令面板、缩放、窄窗口及多组实际中间帧断言。网页退出时保留绘制到淡出结束，`inert` 只禁用交互；减少动态效果同时验证设备宽度和原生面板无补间。工具 hook、命令与稳定预览宿主的独立测试合计 **49 项通过**；最终 desktop 全量 **247 个文件、2,509 项通过、2 项跳过**，三端类型检查、desktop 构建、Hooks、shadow contract 与文档链接检查通过，CSS 变量检查仍为原有 5 处提示。
