...
async function main() {
  ...
  try {
    ...
    const stageBox = await loader.locator('button').boundingBox()
    if (stageBox) {
      await win.mouse.move(stageBox.x + stageBox.width / 2, stageBox.y + stageBox.height / 2)
      await win.mouse.down()
      await win.mouse.move(stageBox.x + stageBox.width / 2 + 48, stageBox.y + stageBox.height / 2)
      await win.mouse.up()
    }
    check('滑动只改变方向，不会被尾随 click 误判为退出', await loader.getAttribute('data-image-generation') === 'snake')
    ...
    await loader.locator('button').click()
    await win.waitForFunction(() => document.querySelector('.image-generation-loader')?.getAttribute('data-image-generation') === 'ambient')
    check('再次点击贪吃蛇退出并恢复加载点阵', await loader.getAttribute('data-image-generation') === 'ambient')
    // 再进一次游戏，随后验证停止任务仍能清场。
    await loader.locator('button').click()
    await win.waitForFunction(() => document.querySelector('.image-generation-loader')?.getAttribute('data-image-generation') === 'snake')
