...
const CUSTOM = [
  { label: 'purple', accent: '#8b7fd6' },
  { label: 'white', accent: '#ffffff' },
  { label: 'black', accent: '#000000' },
  { label: 'red', accent: '#ff0000' },
  { label: 'lime', accent: '#00ff00' },
  { label: 'black-bg', accent: '#8b7fd6', bg: '#000000' },
  { label: 'white-bg', accent: '#8b7fd6', bg: '#ffffff' },
]
...
;(async () => {
  ...
  const combos = [
    ...
    ...CUSTOM.map((item) => ({ skin: 'custom', label: `custom:${item.label}`, vars: item })),
  ]
  const failures = []
  ...
  for (const lang of LANGS) {
    for (const combo of combos) {
      for (const mode of ['light', 'dark']) {
        const inlineVars = combo.vars ? customSkinVars(combo.vars.accent, mode === 'dark', combo.vars.bg) : null
        await page.evaluate(({ lang, skin, mode, inlineVars }) => {
        ...
        }, { lang, skin: combo.skin, mode, inlineVars })
        ...
        const prefix = `${lang}/${combo.label}/${mode}`
  ...
  await browser.close()
  const total = LANGS.length * combos.length * 2
  const matrixOk = pass(`主题可读性矩阵：${total} 个语言×配色×明暗组合`, failures.length === 0,
    failures.length ? failures.slice(0, 10).join(' ; ') + (failures.length > 10 ? ` …共 ${failures.length} 条` : '')
      : `最低 ${worst.ratio.toFixed(2)}:1 @ ${worst.id}`)
  process.exit(axisOk && semanticOk && fillSemanticOk && surfaceInkOk && previewOk && matrixOk ? 0 : 1)
})().catch((error) => {
  console.error(error)
  process.exit(1)
})
