...
async function main() {
  ...
  // ── 1 Amadeus 右栏默认 = 对话 ────────────────────────────────────────────────
  const spaceBtn = win.locator('.rb-space[aria-label="Note"], .rb-space:has-text("Note")').first()
  ...
  await w2.locator('.rb-space[aria-label="Note"], .rb-space:has-text("Note")').first().click().catch(() => {})
