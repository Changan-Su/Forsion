 ...
 * `--source winget` 顺带绕开 msstore 源,那份协议根本不会出现。
 */
...
export function wingetInstall(id: string): string {
  return `winget install --id ${id} -e --source winget --accept-source-agreements --accept-package-agreements --disable-interactivity`
}
...
const COMMANDS: Record<string, Partial<Record<InstallPlatform, (cn: boolean) => string>>> = {
  ...
  docker: {
    ...
    linux: (cn) => cn ? 'curl -fsSL https://get.docker.com | sh -s -- --mirror Aliyun' : 'curl -fsSL https://get.docker.com | sh',
