 ...
 *   私聊(soloAgentSlug / soloEngineId):头像 + 名字 + 「工作区 = Library · 历史会话仅 Agent 可读」+ 「新会话(先总结记忆)」+ 「组建团队」(Agent 私聊才有,P5d)
...
export function OrbitBar({ sessionId, cfg, running }: { sessionId: string; cfg: AgentConfig; running: boolean }): React.ReactElement | null {
  ...
  const memberNames = members.map(nameOf)
    ...
    <div className="t2o-bar" role="status" data-orbit={isTeam ? 'team' : 'teammode'}>
      ...
      <span className="t2o-bar-avatars">
        {members.slice(0, 6).map((slug) => {
          ...
          // 首字中性底色 + 成员间去撞字(09-25 拍板)。
          return <AgentAvatar key={slug} name={nameOf(slug)} url={url} siblings={memberNames} size={18} title={nameOf(slug)} className={cls} initialClassName="t2o-bar-avatar-text" data-work={st === 'working' || st === 'waiting' ? st : undefined} />
        })}
      </span>
      <span className="t2o-bar-acts">
