...
//   R1 编译字节经真解码器进 GridView,19 张真卡都上屏(6 KPI + 10 top5 + 3 section)
...
;(async () => {
  ...
  try {
    ...
    check('R1 编译字节 → 真 GridView:19 张卡全部上屏(6 KPI+10 top5+3 section)', state.cards === 19, `cards=${state.cards}`)
    check('R1b 其中 16 张是 stat 卡(6 KPI + 10 top5)', state.stats === 16, `stats=${state.stats}`)
    check('R2 literal 值上屏(1,234 / 98.6% / 123,456,789)',
      state.text.includes('1,234') && state.text.includes('98.6%') && state.text.includes('123,456,789'))
    check('R2b 无 .db 残影(不出现 加载中/找不到)', !state.text.includes('找不到「') && !state.text.includes('加载中'))
    check('R3 三条分区标题全对', ['服务器状态 · demo-host', '模型用量 TOP 5(30天)', '用户用量 TOP 5(30天)'].every((s) => state.text.includes(s)))
    ...
    check('R4 暗色档同样 19 张卡', dark.cards === 19, `cards=${dark.cards}`)
