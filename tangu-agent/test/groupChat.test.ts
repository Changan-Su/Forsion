vi.mock('../src/services/historianSession.js', () => ({ completeHistorianTask: vi.fn(async () => ({ content: 'HOST-SUMMARY', model: {}, usage: { prompt_tokens: 10, completion_tokens: 5 }, historianSessionId: 'hist-1' })) }));
...
beforeEach(() => {
  vi.mocked(completeHistorianTask).mockClear();
...
function params(over: Partial<any> = {}): any {
  return {
    ...
    agentConfig: { groupNoSummary: true, groupAgents: ['alpha', 'beta'], groupMaxRounds: 2, groupSeedHistory: false },
...
describe('runGroupChat', () => {
  ...
  it('every member ending with DONE stops the discussion (no vote step, no vote events)', async () => {
    ...
    await runGroupChat(params({ agentConfig: { groupNoSummary: true, groupAgents: ['alpha', 'beta'], groupMaxRounds: 5 } }));
  ...
  it('duplicate slugs are deduped in order (TUI `/groupchat a a b`): 2 participants, no crash', async () => {
    doneDecider = () => true;
    await runGroupChat(params({ agentConfig: { groupNoSummary: true, groupAgents: ['alpha', 'alpha', 'beta'], groupMaxRounds: 3 } }));
  ...
  it('explicit groupMaxRounds stays a hard cap when nobody says DONE (internal callers: discussion / historian assist)', async () => {
    doneDecider = () => false;
    await runGroupChat(params({ agentConfig: { groupNoSummary: true, groupAgents: ['alpha', 'beta'], groupMaxRounds: 3 } }));
  ...
  it('no groupMaxRounds = no default cap: members keep going until they all say DONE (well past the old default of 7 rounds)', async () => {
    ...
    await runGroupChat(params({ agentConfig: { groupNoSummary: true, groupAgents: ['alpha', 'beta'] } }));
    expect(endedPayload()).toMatchObject({ reason: 'done', steps: 19 });
  });

  it('errors (no done) with fewer than 2 valid participants', async () => {
    await runGroupChat(params({ agentConfig: { groupNoSummary: true, groupAgents: ['alpha'], groupMaxRounds: 2 } }));
  ...
  it('Historian attaches a summary without asking or posting a moderator speech', async () => {
    doneDecider = () => true;
    await runGroupChat(params({ agentConfig: { groupAgents: ['alpha', 'beta'], groupMaxRounds: 3 } }));
    expect(finals.at(-1)?.content).toBe('**📋 Historian**\n\nHOST-SUMMARY');
    expect(events.some((e) => e.type === 'inquiry_request')).toBe(false);
    expect(events.some((e) => e.type === 'group_speaker' && e.payload.slug === '__host__')).toBe(false);
    expect(events.find((e) => e.type === 'group_summary')?.payload).toMatchObject({ text: 'HOST-SUMMARY', historianSessionId: 'hist-1' });
    expect(completeHistorianTask).toHaveBeenCalledWith(expect.objectContaining({ sessionId: 's1', task: 'team-summary' }));
  });

  it('summary failure does not fail completed team work', async () => {
    vi.mocked(completeHistorianTask).mockRejectedValueOnce(new Error('provider unavailable'));
    doneDecider = () => true;
    await runGroupChat(params({ agentConfig: { groupAgents: ['alpha', 'beta'] } }));
    expect(statuses.at(-1)?.status).toBe('done');
    expect(events.some((e) => e.type === 'inquiry_request' || e.type === 'error')).toBe(false);
  });
  ...
  it('temporary agents participate alongside saved agents (not on disk)', async () => {
    ...
    await runGroupChat(params({ agentConfig: { groupNoSummary: true, groupAgents: ['alpha', 'temp-x'], groupTempAgents: [temp], groupMaxRounds: 1 } }));
    expect(finals.map((f) => f.content)).toEqual([
      '**🗣 Alpha**\n\nalpha-speech-1',
      '**🗣 Gamma**\n\ntemp-x-speech-1',
    ]);
  ...
  it('drops invalid temp agents (missing required fields) → <2 errors', async () => {
    ...
    await runGroupChat(params({ agentConfig: { groupNoSummary: true, groupAgents: ['alpha', 'temp-bad'], groupTempAgents: [bad], groupMaxRounds: 2 } }));
    expect(events.find((e) => e.type === 'error')?.payload.error).toBe('group_needs_2_agents');
  });
});
