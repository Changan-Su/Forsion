...
// 09-26 合并 origin 后:托管参数(含本机沙箱)住在「常规 › 本机运行环境」子页(g-runtime),只在已落盘为托管时渲染;
// 运行方式卡与外部连接仍在「连接」页。tab 走 resolveSettingsTarget 的 `${tab}/${sub}` 深链。
async function openSettingsWin(app, win, tab = 'connection') {
  await win.evaluate(([n, t]) => window.tangu.openFloatingPanel({ id: 'settings', title: 'Settings', builtin: 'settings', params: { tab: t, n } }), [Date.now(), tab])
  let sw
...
async function main() {
  ...
  try {
    ...
    // 连接页就地给显式切换(托管参数不在这一页);切完后端起来,再去「本机运行环境」改沙箱。
    assert.equal(await sw.locator('.settings-runtime-panel').count(), 0, 'connection page no longer embeds the runtime panel')
    await sw.locator('.settings-body .special-save button').filter({ hasText: '切换到托管并启动' }).click()
    await poll(async () => (await win.evaluate(() => window.tangu.getConfig())).mode === 'managed', 'explicit switch persists mode=managed')
    await waitReady(win)
    sw = await openSettingsWin(app, win, 'general/g-runtime')
    ...
    await sw.locator('.settings-runtime-panel button').filter({ hasText: '保存并重启' }).click()
    ...
    await win.waitForSelector('.dv-groupview', { timeout: 30_000 })
    await win.locator('#tangu-splash').waitFor({ state: 'detached' })
    sw = await openSettingsWin(app, win, 'general/g-runtime'); panel = sw.getByTestId('host-sandbox-settings')
    await panel.waitFor()
    ...
    await waitReady(win)
    sw = await openSettingsWin(app, win, 'connection')
