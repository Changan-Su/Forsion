...
const people=[['arioso','Arioso','综合判断 · 组织提纲'],['aria','Aria','探索表达 · 展开可能'],['recita','Recita','核查证据 · 审视假设']]
const agents=people.map(([slug,name,description])=>({slug,name,description,createdBy:'user',model:'m1',tools:[],systemPrompt:description,soul:description,libraryDir:path.join(home,'agents',slug,'Library')}))
for(const a of agents)fs.mkdirSync(a.libraryDir,{recursive:true})
const team={slug:'research-team',name:'研究分享 TEAM',description:'展开方向，检查依据，把结论带回项目。',avatar:'',lead:'arioso',members:people.map(([slug,,role])=>({slug,role})),doc:'# 共同目标\n\n完成一场有依据、可继续推进的研究分享。\n\n## 协作约定\n\n- 每项结论保留来源。\n- 分开事实、推断与待核实项。\n- 公开同步发现，将产物写回项目。',createdAt:at,libraryDir:path.join(home,'teams/research-team/Library')}
...
const solo={...base,id:'showcase-memory',title:'偏好与项目背景',agent_config:{agentSlug:'arioso',execMode:'host',cwd:vault}}
...
async function mainRun(){
 ...
 const stub=await startStubEngine({agents,sessions:[main,solo],messages,models:[{id:'m1',name:'公开演示 · 无模型调用',provider:'fixture',contextWindow:128000}],override:async({path:p,method,url,body:readBody})=>{
  ...
  if(p==='/agent/agents-meta')return {defaultSlug:'arioso',order:people.map(x=>x[0])}
 ...
 for(const d of ['userData','userData-dev']){fs.writeFileSync(path.join(home,d,'tangu-desktop-config.json'),JSON.stringify({mode:'external',backendUrl:stub.url,token:'public-showcase',defaultWorkspaceDir:vault}));fs.writeFileSync(path.join(home,d,'amadeus-config.dev.json'),JSON.stringify({localVault:vault,lastVault:vault}))}
 let app,win
 try{
  app=await electron.launch({args:[`--user-data-dir=${path.join(home,'userData')}`,'--lang=zh-CN',ROOT],cwd:ROOT,env:{...process.env,TANGU_HOME:home,TANGU_BACKEND_URL:stub.url},timeout:45000})
  ...
  if(scene==='note'||scene==='blocks'){
   ...
   await space('Agents');await win.locator('[data-agents-space]').waitFor();await win.locator('.agents-roster-item').filter({hasText:'Arioso'}).first().click();await win.locator('[data-agent-profile="arioso"]').getByRole('tab',{name:'记忆',exact:true}).click();await win.locator('[data-memory-entry-id]').first().waitFor()
  }else if(scene==='team'){
   await win.locator('.t2s-srow, .t2o-row').filter({hasText:team.name}).first().click();await win.locator('[data-team-desk="status"]').waitFor();await win.waitForTimeout(600);await win.screenshot({path:path.join(OUT,'team-overview.png'),animations:'disabled',scale:'css'});await win.locator('[data-team-desk="status"] [data-slug="recita"]').click();await win.locator('.child-chat-panel').waitFor()
  }else if(scene==='muse'){
  ...
  fs.writeFileSync(path.join(OUT,scene+'.json'),JSON.stringify({scene,sourceCommit:execFileSync('git',['rev-parse','HEAD'],{cwd:REPO,encoding:'utf8'}).trim(),version:require(path.join(ROOT,'package.json')).version,home,requests,liveModelCalls:0,publicFixtures:true,viewport:await win.evaluate(()=>({width:innerWidth,height:innerHeight})),bodyText:await win.locator('body').innerText()},null,2))
