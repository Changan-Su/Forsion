 ...
 *      再按设置页环境检测的写法(不走 shell)从带空格的路径调一次。
 *
...
try {
  ...
  // 设置页环境检测的写法(main.ts probeVersion(…, false)):execFile 不走 shell。模拟装进 Program Files 这类
  // 带空格的目录 —— 借一个带空格的目录联接(runner 一次性,联接不收拾)。
  const spaced = path.join(os.tmpdir(), 'Forsion Probe Git');
  fs.symlinkSync(path.join(resources, 'git'), spaced, 'junction');
  const direct = execFileSync(path.join(spaced, 'cmd', 'git.exe'), ['--version'], { encoding: 'utf8', windowsHide: true });
  console.log(`  带空格路径直接 execFile:${direct.trim()}`);
  results.git = /^git version/.test(out.trim()) && /^git version/.test(direct.trim());
