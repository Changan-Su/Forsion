...
export function FloatingPanelFrame({ title, onClose, children, compact = false, narrow = false }: {
  title: string
  onClose: () => void
  children: ReactNode
  compact?: boolean
  ...
  return <motion.div className="floating-panel-layer" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
    ...
    <motion.section className={`floating-panel-window${compact ? ' compact' : ''}${narrow ? ' narrow' : ''}`} role="dialog" aria-modal="true" aria-label={title}
