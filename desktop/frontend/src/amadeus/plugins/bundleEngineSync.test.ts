...
const calls = { puts: [] as Array<[string, boolean]>, lists: 0, engine: [] as Array<{ id: string; enabled: boolean; source: string }> }
vi.mock('../../services/backendService', () => ({
  listPlugins: vi.fn(async () => { calls.lists += 1; return calls.engine }),
  setPluginEnabled: vi.fn(async (_cfg: unknown, id: string, enabled: boolean) => { calls.puts.push([id, enabled]); return { ok: true, enabled } }),
