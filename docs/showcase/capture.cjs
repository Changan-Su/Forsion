 ...
 * Build desktop, then: node docs/showcase/capture.cjs [note|blocks|memory|team|muse] [zh|en]
...
const en = require('./fixture.en.cjs')
const local = (zh, english) => locale === 'en' ? english : zh
const projectName = local('研究分享',en.project)
const OUT = process.env.SHOWCASE_OUT || path.join(os.tmpdir(), 'forsion-showcase-v211'+(locale==='en'?'-en':''))
const scene = process.argv[2] || 'note'
const home = fs.mkdtempSync(path.join(os.tmpdir(), 'forsion-public-showcase-'))
const vault = path.join(home, projectName)
const at = '2026-09-17 10:00:00'
const stamp = Date.parse('2026-09-17T10:00:00Z')
const note = local(`# 把一次思考，变成下一次的起点
...
fs.writeFileSync(path.join(vault,projectName+'.md'),note)
fs.writeFileSync(path.join(vault,local('研究问题',en.questionTitle)+'.md'),local('# 研究问题\n\n如何让收集的资料接上下一次工作？\n\n观察：背景要重复交代，结论留在聊天，后续容易遗忘。\n',en.question))
fs.writeFileSync(path.join(vault,local('分享提纲',en.outlineTitle)+'.md'),local('# 分享提纲\n\n## 记录\n把材料连接起来。\n\n## 思考\n让不同角色检查判断。\n\n## 行动\n保存成果，安排后续复查。\n',en.outline))
const people=[['xyra','Arioso','综合判断 · 组织提纲'],['aria','Aria','探索表达 · 展开可能'],['recita','Recita','核查证据 · 审视假设']]
for(let i=0;i<people.length;i++)people[i][2]=local(people[i][2],en.roles[i])
...
const team={slug:'research-team',name:local('研究分享 TEAM',en.teamName),description:local('展开方向，检查依据，把结论带回项目。',en.teamDescription),avatar:'',lead:'xyra',members:people.map(([slug,,role])=>({slug,role})),doc:'# 共同目标\n\n完成一场有依据、可继续推进的研究分享。\n\n## 协作约定\n\n- 每项结论保留来源。\n- 分开事实、推断与待核实项。\n- 公开同步发现，将产物写回项目。',createdAt:at,libraryDir:path.join(home,'teams/research-team/Library')}
team.doc=local(team.doc,en.teamDoc)
fs.mkdirSync(team.libraryDir,{recursive:true})
fs.writeFileSync(path.join(home,'teams/research-team/TEAM.md'),team.doc)
const base={summary:'',archived:false,model_id:'m1',created_at:at,updated_at:at,project_path:vault,project_name:projectName,projectless:false}
...
const main={...base,id:'showcase-team',title:local('研究分享 · 分工与成果',en.mainTitle),agent_config:config}
const solo={...base,id:'showcase-memory',title:local('偏好与项目背景',en.soloTitle),agent_config:{agentSlug:'xyra',execMode:'host',cwd:vault}}
const children=people.map(([slug,name])=>({...base,id:`showcase-${slug}`,title:`${name} · ${projectName}`,agent_config:{agentSlug:slug,execMode:'host',cwd:vault,teamMember:{teamSessionId:main.id}}}))
...
for(const [i,m] of messages.entries())m.content=local(m.content,en.messages[i])
for(const [i,m] of childMessages.entries())m.content=local(m.content,en.childMessages[i])
const facts=local(['做研究时，先给结论，再列来源；不确定的内容单独标出。','当前项目：准备 15 分钟研究分享，围绕记录、思考与行动组织提纲。','审阅后把提纲、依据和下一步写回项目笔记，保留来源链接。'],en.facts)
...
for(const [i,m] of mails.entries())m.title=local(m.title,en.mailTitles[i])
mails[1].body=local(mails[1].body,en.secondMailBody)
...
async function mainRun(){
 const requests=[]
 const stub=await startStubEngine({agents,sessions:[main,solo],messages,models:[{id:'m1',name:local('公开演示 · 无模型调用',en.modelName),provider:'fixture',contextWindow:128000}],override:async({path:p,method,url,body:readBody})=>{
  ...
  if(p==='/agent/special/muse/todos/public-followup')return {todo:{id:'public-followup',title:local('复查分享提纲的来源与待确认项',en.taskTitle),status:'pending'}}
 ...
 let app,win
 try{
  app=await electron.launch({args:[`--user-data-dir=${path.join(home,'userData')}`,`--lang=${locale==='en'?'en-US':'zh-CN'}`,ROOT],cwd:ROOT,env:{...process.env,TANGU_HOME:home,TANGU_BACKEND_URL:backendUrl},timeout:45000})
  ...
  for(const label of ['跳过引导','Skip onboarding','Skip']){const b=win.getByText(label,{exact:true}).first();if(await b.isVisible().catch(()=>false)){await b.click();break}}
  await win.waitForSelector('.dv-groupview')
  await win.evaluate(lang=>{localStorage.setItem('forsion_default_space','tangu');localStorage.setItem('forsion_theme_pref','light');localStorage.setItem('tangu_locale',lang);localStorage.setItem('forsion.ntf.prefs',JSON.stringify({enabled:true,osEnabled:false,events:{'sync.error':false}}));localStorage.setItem('forsion.sb.prefs',JSON.stringify({enabled:true,hidden:['sync.status'],order:[]}))},locale)
  ...
  const assertImages=async()=>{
   if(locale==='en'){const text=await win.locator('body').innerText();if(/[\u3400-\u9fff]/u.test(text))throw Error('Unexpected Chinese text in English capture: '+text.match(/[^\n]*[\u3400-\u9fff][^\n]*/gu)?.join(' | '))}
  ...
  if(scene==='note'||scene==='blocks'){
   await space('Note');await win.locator('.t2s-srow').filter({hasText:projectName}).first().click();await win.locator('.unified-body .ProseMirror h1').first().waitFor(); if(scene==='blocks'){const lead=win.locator('.unified-body .ProseMirror p').filter({hasText:local('用一篇项目笔记',en.lead)}).first();await lead.click();await lead.evaluate(el=>{const range=document.createRange();range.selectNodeContents(el);range.collapse(false);const selection=window.getSelection();selection.removeAllRanges();selection.addRange(range)});await win.keyboard.press('Enter');await win.waitForTimeout(700);await win.keyboard.type('/',{delay:80});await win.waitForTimeout(300);await win.keyboard.type('h',{delay:80});await win.locator('.slash-menu').waitFor();if(locale==='en'&&await lead.innerText()!==en.lead)throw Error('Block insertion changed the lead paragraph')}
  }else if(scene==='memory'){
   await space('Agents');await win.locator('[data-agents-space]').waitFor();await win.locator('.agents-roster-item').filter({hasText:'Arioso'}).first().click();await win.locator('[data-agent-profile="xyra"]').getByRole('tab',{name:local('记忆','Memory'),exact:true}).click();await win.locator('[data-memory-entry-id]').first().waitFor()
  }else if(scene==='team'){
   ...
   await space('收件箱|Inbox');await win.locator('.t2sw-plug-list .t2s-srow').filter({hasText:mails[0].title}).click();await win.getByRole('button',{name:local('交给 Muse 执行','Let Muse do it'),exact:true}).waitFor();await win.getByText(local('查看任务书','Show task brief'),{exact:true}).click()
